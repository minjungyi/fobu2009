...
function viewList(seq){
    ...
    success:function(data, status) {        
        ...
        var cContents = "<tr><td width='730' valign='top' align='center'><table width='98%' border='0' cellspacing='0' cellpadding='0'><tr>"
            ...
            +"<p><select name='memoSel' id='memoSel'><option value='품절'>품절</option><option value='품절주문'>품절주문</option><option value='옵션품절'>옵션품절</option><option value='입금확인'>입금확인</option><option value='입고예정'>입고예정</option>"
...
function search(pageNum, eqNum){
	...
	$.ajax({
        ...
        success:function(data, status) {
			...
			var cHeader = "<tr>"
				+"<td width='95' height='23' align='center' bgcolor='e7e7e7' class='tx12B'>입금|보류|취소</td>"
				+"<td width='30' align='center' bgcolor='e7e7e7' class='tx12B'>발주</td>"
			...
			if(data.totalOrderList.length == 0){
	            ...
	            $.each(data.totalOrderList, function() {
	            	...
	            	c += "<tr onclick='rowSelect(this, \""+this.seq+"\");' bgcolor='#FFFFFF' style=cursor:hand;>"
	      			  +"<td width='95' height='20' align='center'>"
	          		  ...
	          		  +"<input type='checkbox' name='checkbox3' id='checkboxC"+a+"' value='"+this.seq+"' onclick='chkYn(this.name,"+a+");'></td>"
	          		  +"<td width='30' align='center' ><input type='checkbox' name='checkboxOrder' id='checkboxOrder"+a+"' value='"+this.seq+"'></td>"
...
function chkArrOrder(){
    var arrChkVal ="";
    var checkAllAValue = frm.checkAllA.value;
    var checkedCnt = 0;
    var companyCategory = frm.companyCategory.value;
    $("[name=checkboxOrder]").each(function(){
        if($(this).is(":checked")){
            arrChkVal += $(this).val()+","; 
            checkedCnt++;       
        }
    ...
    if(checkedCnt == 0){
    	var enterpriseOrderPopWin = window.open('/admin/product/enterpriseOrder.html?companyCategory='+companyCategory,'enterpriseOrderPop', 'width=800, height=700, toolbar=no, location=no, status=no, menubar=no, scrollbars=yes');
    	enterpriseOrderPopWin.focus();
        return;
    }
    
    frm.orderSeqStr2.value = arrChkVal;
    onOrderStock();
   //alert(frm.orderSeqStr2.value);
}
...
function allChkOrder(objNm){
	var tempNm = "checkboxOrder";
...
function onOrderStock(){
    var orderSeqStrTmp = frm.orderSeqStr2.value;
    ...
    $.ajax({
        ...
        dataType: "json",
        url:"/admin/orderManage/saveEnterOrder.action",
        success:function(data, status) {
        },
        error:function(request, status) {
            alert(status);
        ...
        complete:function(xhr, textStatus) {
            var enterpriseOrderPopWin = window.open('/admin/product/enterpriseOrder.html','enterpriseOrderPop', 'width=800, height=700, toolbar=no, location=no, status=no, menubar=no, scrollbars=yes');
            enterpriseOrderPopWin.focus();
...
function exportExcel(){
	...
	$.ajax({
        ...
        dataType: "json",
        url:"/util/enterpriseOrderExcelEtc.action",
        beforeSend:function(xhr) {
...
function exportExcelImg(){
	...
	document.location.href = "/admin/excel/order_excellist.html?"+paramData;
}
...
<input type="hidden" name="orderSeqStr2">
<input type="hidden" name="orderStatStr">
<input type="hidden" name="pageNumTmp" value="1">
<table width="1200" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td height="28" valign="bottom" class="tx14B" style="padding-left:10px">▣ 업체발주</td>
  </tr>
</table>
<table width="1200" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
...
<table width="1200" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="95" align="center">
    	<input type="checkbox" name="checkAllA" id="checkAllA" value="2" onclick="allChk('checkAllA');">
    	<input type="checkbox" name="checkAllB" id="checkAllB" value="4" onclick="allChk('checkAllB');">
    	<input type="checkbox" name="checkAllC" id="checkAllC" value="5" onclick="allChk('checkAllC');">
    </td>
    <td width="260">
	    <img src="/images/button/tb_check.gif" style="cursor:hand;" alt="체크전송" onclick="chkArrValue();">
	    <input type="checkbox" name="checkAllOrder" id="checkAllOrder" value="5" onclick="allChkOrder('checkAllOrder');">
	    <input type="button" width="100px" style="cursor:hand;height:22px" value="주문발주" onclick="chkArrOrder();"/>
    </td>
    ...
    <td width="355" align="right">
        ...
        <img src="/images/bn_ex_img.gif" style="cursor:hand;" alt="엑셀다운(그림)" height="22" onclick="exportExcelImg();">
        <img src="/images/bn_ex_text.gif" style="cursor:hand;" alt="엑셀다운" height="22" onclick="exportExcel();">
    </td>
  </tr>
</table>
</form>
