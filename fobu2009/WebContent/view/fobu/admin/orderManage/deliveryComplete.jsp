...
$(function() {
    ...
    $('#datepicker3').val(year+"-"+mon+"-"+day);
    $('#datepicker4').val(year+"-"+mon+"-"+day);
...
function onDayPeriod(){
	var dayMinus = $('#dayPeriod').val();
	var now = new Date();
	now.setDate(now.getDate() + dayMinus*1);
...
function viewList(seq){
    ...
    success:function(data, status) {        
        ...
        var cContents = "<tr><td width='700' valign='top' align='center'><table width='98%' border='0' cellspacing='0' cellpadding='0'><tr>"
	    	...
	    	+"<p><select name='memoSel' id='memoSel'><option value='입고예정'>입고예정</option><option value='품절'>품절</option><option value='옵션품절'>옵션품절</option><option value='발주예정'>발주예정</option>"
...
function search(pageNum, eqNum){
	...
	$.ajax({
        ...
        success:function(data, status) {
			...
			var cHeader = "<tr>"
				+"<td width='72' height='23' align='center' bgcolor='e7e7e7' class='tx12B'>교환|반품</td>"
				+"<td width='30' align='center' bgcolor='e7e7e7' class='tx12B'>PR</td>"
				+"<td width='30' align='center' bgcolor='e7e7e7' class='tx12B'>수집</td>"
				+"<td width='50' align='center' bgcolor='e7e7e7' class='tx12B'>택배</td>"
				+"<td width='100' align='center' bgcolor='e7e7e7' class='tx12B'>송장번호</td>"
				+"<td width='85' align='center' bgcolor='e7e7e7' class='tx12B'>배송완료일</td>"
			...
			if(data.totalOrderList.length == 0){
	            ...
	            $.each(data.totalOrderList, function() {
	            	...
	            	c += "<tr onclick='rowSelect(this, \""+this.seq+"\");' bgcolor='#FFFFFF' style=cursor:hand;>"
	          		  ...
	          		  +"<td width='30' align='center' ><img src='/images/com/"+companyImage+".gif' ></td>"
	      			  +"<td width='30' align='center' ><a href='"+marketTypeUrl+this.url+"'  target='_blank'><img src='/images/"+this.siteImg+".gif'></a></td>"
	      			  +"<td width='60' align='center' >"+deliverCom+"</td>"
	       			  +"<td width='100' align='center' ><a href='http://www.cjgls.co.kr/kor/service/service02_01.asp?slipno="+this.deliveryNumber.replace(/-/g,'')+"'  target='_blank'>"+this.deliveryNumber+"</a></td>"
	      			  +"<td width='85' align='center' >"+this.deliveryDt+"</td>"
	      			  ...
	      			  +"<td align='left' ><div style='width:350px; text-overflow:ellipsis; overflow:hidden; white-space:nowrap;'>["+this.brandNm+"]"+this.product+"</div></td>"
	      			  +"<td width='30' align='right' ><b>"+commify(this.orderQy)+"</b></td>"
	      			  //+"<td width='30' align='right' ><font color="+fontColor2+">"+commify(this.productOrderQySum)+"</font>/<a href='javascript:changeRealSrock(\""+this.productCode+"\", \""+this.product+"\", \""+this.realStock+"\");'>"+commify(this.realStock)+"</a></td>"
...
function exportExcel(){
	var siteCode = frm.siteCode.value;
	var startDay = frm.startDay2.value;
	var endDay = frm.endDay2.value;
	var nameSel = frm.nameSel.value;
	var nameText = frm.nameText.value;
	var companyCategory = frm.companyCategory.value;
	var shopId = frm.shopId.value;
	...
	$.ajax({
        ...
        dataType: "json",
        url:"/util/enterpriseDepositExcel.action",
        beforeSend:function(xhr) {
        	showMsg("데이터를 엑셀로 변환하고 있습니다.<br><br>잠시만 기다려 주세요!");
		},
        success:function(data, status) {
			$("#fileDown").attr("src","/util/download.action?key=&fileName=enterpriseDepositExcel.xls&orgFileName=enterpriseDepositExcel.xls");
        },
        error:function(request, status) {
           alert(status);
        },
...
<input type="hidden" name="startDay2" id="datepicker3" readonly>
<input type="hidden" name="endDay2" id="datepicker4" readonly>
...
<table width="1200" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td height="28" valign="bottom" class="tx14B" style="padding-left:10px">▣ 배송완료</td>
  </tr>
</table>
<table width="1200" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
...
<table width="1200" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="72" align="center">
    	<input type="checkbox" name="checkAllA" id="checkAllA" value="9" onclick="allChk('checkAllA');">
    	<input type="checkbox" name="checkAllB" id="checkAllB" value="7" onclick="allChk('checkAllB');"></td>
    <td width="80"><img src="/images/button/tb_check.gif" onclick="chkArrValue();"></td>
    <td width="60">
        <select name="dayPeriod" id="dayPeriod" onchange="onDayPeriod();">
    ...
    <td width="80"><img src="/images/bn_ex_expshop.gif" height="22" onclick="exportExcel();"></td>
    ...
    <td width="292"><iframe id='fileDown' src='about:blank;' frameborder='0' width=0 height=0></iframe></td>
  </tr>
</table>
</form>
<!-- top start -->
