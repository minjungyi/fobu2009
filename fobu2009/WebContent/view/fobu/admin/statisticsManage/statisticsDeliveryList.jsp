...
function search(pageNum){
	...
	$.ajax({
        ...
        dataType: "json",
        url:"/admin/statisticsManage/initDelivery.action",
        ...
        success:function(data, status) {
			...
			var cHeader = "<tr>"
				+"<td width='200' height='23' align='center' bgcolor='e7e7e7' class='tx12B'>날짜</td>"
				+"<td width='225' align='center' bgcolor='e7e7e7' class='tx12B'>배송 총건수</td>"
				+"<td width='225' align='center' bgcolor='e7e7e7' class='tx12B'>배송완료 건수</td>"
				+"<td width='225' align='center' bgcolor='e7e7e7' class='tx12B'>교환완료 건수</td>"
				+"<td width='225' align='center' bgcolor='e7e7e7' class='tx12B'>반품완료 건수</td>"
				+"</tr>";
			$("#search_total_order").append(cHeader);
			var a = 1;  

			if(data.statisticsDeliveryList.length == 0){
            	c = "<tr><td colspan='17' height='50' bgcolor='#FFFFFF' align='center'><B>해당 데이터가 없습니다.</B></td></tr>";
          		$("#search_total_order").append(c);
          		$("#view_order_detail tr").remove();
            }else{
                
	            $.each(data.statisticsDeliveryList, function() {
	               c = "<tr onclick='rowSelect(this, \""+this.seq+"\");' bgcolor='#FFFFFF' style=cursor:hand;>"
	      			  +"<td height='23' align='center'>"+this.collectDt+"</td>"
	      			  +"<td align='center' >"+(parseInt(this.deliveryCompleteCnt)+parseInt(this.exchangeCnt))+"</td>"
	      			  +"<td align='center' >"+commify(this.deliveryCompleteCnt)+"</td>"
	      			  +"<td align='center' >"+commify(this.exchangeCnt)+"</td>"
	      			  +"<td align='center' >"+commify(this.returnCnt)+"</td>"     			  
	      			  +"</tr>";
	            	$("#search_total_order").append(c);
	            	a++;
	            });
            }
            var totalPage = Math.ceil(parseInt(data.statisticsDeliveryCount)/parseInt(listScale));
            $("#spanCount").empty().append("전체 "+data.statisticsDeliveryCount+" 개 : "+pageNum+" / "+totalPage+" 페이지")
...
<table width="1200" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td height="28" valign="bottom" class="tx14B" style="padding-left:10px">▣ 배송별통계</td>
  </tr>
</table>
<table width="1200" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
